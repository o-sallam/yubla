...
// For Vercel serverless, use /tmp for writable storage
const DB_DIR = process.env.VERCEL ? '/tmp' : (process.env.DB_DIR ? path.resolve(process.env.DB_DIR) : path.resolve(process.cwd(), 'data'));
const DB_PATH = path.join(DB_DIR, 'yubla.sqlite');
const SEED_DB_PATH = process.env.VERCEL ? path.resolve(process.cwd(), 'yubla.sqlite') : DB_PATH;
...
const initDb = async () => {
  ...
  // For Vercel, copy seed database to /tmp on first run
  if (process.env.VERCEL && !fs.existsSync(DB_PATH) && fs.existsSync(SEED_DB_PATH)) {
